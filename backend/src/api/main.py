...
import httpx
import re
# regulation_normalizer import removed - not needed since interpretation is pre-computed
...
async def get_blockfaces(lat: float, lng: float, radius_meters: int = 500):
    ...
    try:
        ...
        async for doc in db.street_segments.find(query):
            ...
            # Pass through all rules - descriptions should already be in MongoDB from ingestion
            rules = doc.get("rules", [])
            ...
            segment_response = {
                ...
                "restrictions": formatted_rules,  # Map rules to restrictions for frontend compat
                
                # Pass through the entire interpretation object from MongoDB
                # This contains rules_display as an array of strings, not objects
