...
Fast version that processes all CNNs without limit.
"""
import os
import asyncio
import csv
...
async def main():
    ...
    # Get all segments at once for efficiency
    print("Step 2: Loading all street segments...")
    all_segments = await db.street_segments.find({}).to_list(None)
    
    # Index segments by CNN
    segments_by_cnn = defaultdict(list)
    for seg in all_segments:
        cnn = seg.get('cnn')
        if cnn:
            segments_by_cnn[cnn].append(seg)
    
    print(f"Loaded {len(all_segments)} segments for {len(segments_by_cnn)} CNNs\n")
    
    print("Step 3: Checking which missing sides have blockface but no parking rules...")
    ...
    for item in cnns_missing_side:
        ...
        processed += 1
        if processed % 100 == 0:
            print(f"  Processed {processed}/{len(cnns_missing_side)} CNNs...")
        
        # Get segments for this CNN
        segments = segments_by_cnn.get(cnn, [])
    ...
    print(f"\nFound {len(manual_verification_needed)} blocks requiring manual verification\n")
    
    # Sort by street name for easier manual verification
    manual_verification_needed.sort(key=lambda x: (x['street_name'], x['from_address']))
